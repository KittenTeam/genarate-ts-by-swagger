...
function primitive(schema) {
    ...
    if (utils.isArray(type)) {
        return schema.type.map(function (type) { return primitives[type] ? primitives[type] : type; }).join('|');
    }
...
function sampleFromSchema(schema) {
    ...
    if (schema['enum']) {
        return utils.normalizeArray(schema['enum']).map(function (item) { return "'" + item + "'"; }).join('|');
        ...
        if (spec.swaggerVersion) { // v1
            ...
            return Promise.all(paths).then(function (apis) {
                ...
                return new Promise(function (resolve, reject) {
                    for (var _i = 0, specs_1 = specs; _i < specs_1.length; _i++) {
                        var spec_1 = specs_1[_i];
                        renameTypeKey(spec_1, spec_1.models);
                    }
            ...
            for (var path in spec.paths) {
                for (var method in spec.paths[path]) {
                    ...
                    if (!api.parameters)
                        continue;
                    for (var _i = 0, _a = api.parameters; _i < _a.length; _i++) {
                        var parameter = _a[_i];
